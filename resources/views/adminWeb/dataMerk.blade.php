...
<head>
    ...
    <title>Data Merk | Admin M-GOLEM</title>
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <strong class="card-title">Data Merk <button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#addData"><i class="fa fa-plus-circle"></i> Add</button></strong>
                            </div>
                            <div class="card-body">
                                    ...
                                    <tbody>
                                      @foreach($merk as $key => $data)
                                        <tr>
                                          ...
                                          <td>
                                            <button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#editData" data-id_merk = "{{ $data->id_merk }}" data-nama_merk="{{ $data->nama_merk }}"><i class="fa fa-edit"></i> Edit</button>
                                            <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#deleteData" data-id_merk = "{{ $data->id_merk }}"><i class="fa fa-trash"></i> Delete</button>
                                          </td>
                                        </tr>
                                      @endforeach
                                    </tbody>
          ...
          <div class="modal-dialog">
            ...
            <div class="modal-content">
              <div class="modal-header">
                ...
                <h4 class="modal-title"><span class="fa fa-plus-circle"></span> Add Data Merk</h4>
              </div>
              <div class="modal-body">
                <form id="modal-form-add" action="{{ route('data-merk.store') }}" method="post" role="form">
                  {{ csrf_field() }}
                  <div class="form-group has-success">
                    <label for="id_merk" class="form-control-label">Id Merk</label>
                    <input type="text" id="id_merk" name="id_merk" class="form-control" required/>
                    <span class="text-warning" ></span>
                  </div>
                  <div class="form-group has-success">
                    <label for="nama_tipe" class="form-control-label">Nama Merk</label>
                    <input type="text" id="nama_merk" name="nama_merk" class="form-control" required />
                  </div>
              </div>
          ...
          <div class="modal-dialog">
            ...
            <div class="modal-content">
              <div class="modal-header">
                ...
                <form id="modal-form-edit" method="post" action="{{ route('data-merk.update', 'update') }}">
                    {{ method_field('patch') }}
                    {{ csrf_field() }}
              <div class="modal-body">
                  ...
                  <div class="form-group has-warning">
                    <label for="nama_merk" class="form-control-label">Nama Merk</label>
                    <input type="text" id="nama_merk" name="nama_merk" class="form-control"  required />
                  </div>
              </div>
              <div class="modal-footer">
          ...
          <div class="modal-dialog">
            ...
            <div class="modal-content">
              <div class="modal-header">
                ...
                <form id="modal-form-delete" method="post" action="{{ route('data-merk.destroy', 'destroy') }}">
                    {{ method_field('delete') }}
                    {{ csrf_field() }}
              <div class="modal-body">
                    <input type="hidden" name="id_merk" id="cat_id" value="">
        ...
        $(document).ready(function() {
          ...
          formAdd.submit(function (e) {
                    ...
                    if( res.error == 0 ){
                      ...
                      swal(
                        ...
                        res.message,
                          ...
                          if(OK){
                            window.location.href = "{{ route('dataMerkAdmin') }}";
                          }
                        });
                    } else{
                        $('#addData').modal('hide');
            ...
            $('#editData').on('show.bs.modal', function (event) {
              ...
              var id_merk   = button.data('id_merk') // Extract info from data-* attributes
              var nama_merk = button.data('nama_merk')

              // If necessary, you could initiate an AJAX request here (and then do the updating in a callback).
              // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
              var modal = $(this)
              modal.find('.modal-body #id_merk').val(id_merk)
              modal.find('.modal-body #nama_merk').val(nama_merk)
              modal.find('.modal-body #cat_id').val(id_merk)
            ...
            $('#deleteData').on('show.bs.modal', function (event) {
              ...
              var id_merk = button.data('id_merk')
              var modal      = $(this)
              modal.find('.modal-body #cat_id').val(id_merk)
            });
